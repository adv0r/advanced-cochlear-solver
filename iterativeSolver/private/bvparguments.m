...
%   $Revision: 1.1.6.1 $  $Date: 2007/05/23 18:54:19 $
    ...
    % BVP5C does not support multipoint BVP    
    if (nregions > 1) && strcmp(solver_name,'bvp5c')
        msg = sprintf('Double entries detected in SOLINIT.x. Use BVP4C for solving multipoint BVP.');
        error('MATLAB:bvparguments:MultipointBVPNotSupported','%s  %s',errIntro,msg); 
    end        
