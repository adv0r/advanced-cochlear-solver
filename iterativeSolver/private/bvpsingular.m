...
%   $Revision: 1.1.6.1 $  $Date: 2007/05/23 18:54:23 $
    ...
    if ~isempty(ST)
        if (solinit.x(1) ~= 0) || (solinit.x(end) <= solinit.x(1))
            error('MATLAB:bvpsingular:SingBVPInvalidInterval',...
                  'Singular BVP must be on interval [0, b] with 0 < b.')
        end    
        if ~isnumeric(ST) || any( size(ST)~=neqn)
            error('MATLAB:bvpsingular:SingBVPInvalidS',...
                  'Singular term must be square matrix of size neqn.')
        end
