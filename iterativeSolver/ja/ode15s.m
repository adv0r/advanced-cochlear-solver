...
%
%   [TOUT,YOUT] = ODE15S(ODEFUN,TSPAN,Y0) は、TSPAN = [T0 TFINAL] のとき、
%   初期条件 Y0 で、時刻 T0 から TFINAL まで微分方程式 (システム) y' = f(t,y) 
...
%   
%   [TOUT,YOUT] = ODE15S(ODEFUN,TSPAN,Y0,OPTIONS) は、デフォルトの積分
...
%   解の一部の成分が、非負でなければなない 場合、その成分のインデックスを 
%   'NonNegative' プロパティに設定するために ODESET を使用してください。 
...
%   スパース行列になります。       
%
%   ODE15S は、は、正則でない質量行列 M(t,y) をもつ問題 M(t,y)*y' = f(t,y) を
...
%
%   [TOUT,YOUT,TE,YE,IE] = ODE15S(ODEFUN,TSPAN,Y0,OPTIONS) は、OPTIONS の
%   'Events' プロパティが関数ハンドル EVENTS に設定されてる場合の解法で、
%   イベント関数と呼ばれる (T,Y) の関数が 0 となる点を求めます。 各関数に
%   対して、積分を 0 で終えるべきかどうか、そしてゼロクロッシングの方向が
%   問題になるかどうか指定します。 これらは、EVENTS: 
%   [VALUE,ISTERMINAL,DIRECTION] = EVENTS(T,Y) によって返される 3 つの
%   ベクトルです。 各 I 番目のイベント関数に対して、VALUE(I) は関数の値を
%   表します。 積分がこのイベント関数の 0 で終了する場合は、関数 
%   ISTERMINAL(I)=1 の値で、そうでない場合は 0 です。 すべての 0 が計算
%   される (デフォルト) 場合は、DIRECTION(I)=0 で、イベント関数が増加した
%   点のみ 0 である場合は +1 で、イベント関数が減少した点のみ 0 である
%   場合は -1 です。 出力 TE は、イベント発生時間の列ベクトルです。 YE の
%   行は対応する解で、ベクトル IE のインデックスはどのイベントが発生したかを
%   示しています。
%   
%   SOL = ODE15S(ODEFUN,[T0 TFINAL],Y0...) は、T0 と TFINAL 間の任意の
%   点における解を計算するため、DEVAL で使用可能な構造体を出力します。
%   ODE15S で選択されたステップは、行ベクトル SOL.x に出力されます。
%   各 I に対して、列 SOL.y(:,I) は、SOL.x(I) での解を含んでいます。 
%   イベントが検出された場合は、SOL.xeは、イベントが発生した位置を示す点
%   から構成される行ベクトルです。 SOL.ye の列は、対応する解で、ベクトル 
%   SOL.ie のインデックスは、どのイベントが発生したかを示しています。
%   ターミナルイベントが検出された場合、SOL.x(end) は、イベントが発生した
%   位置でのステップの終わりを含んでいます。 イベントの正確な位置は、
%   SOL.xe(end) に報告されます。 
%
%   例
